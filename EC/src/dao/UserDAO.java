...
//	user_idに紐付いたt_buyテーブル情報を取得
	public static List<BuyDataBeans> getTbuyByUserId(int userId) throws SQLException {
		List<BuyDataBeans> bdbList = new ArrayList<>();
		Connection con = null;
		PreparedStatement st = null;
		try {
			con = DBManager.getConnection();

			st = con.prepareStatement("select t_buy.*, m_delivery_method.* from t_buy"
